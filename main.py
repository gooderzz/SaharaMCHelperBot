...
# Embed Preparation
# Application Ready Embed
applicationReady = discord.Embed(title="Apply For SaharaMC Whitelist",
                                 description="Are you ready to apply for SaharaMC's Whitelist? (Use reaction to continue)",
                                 color=0x00ff00)
...
applicationCancelled = discord.Embed(title="Application for SaharaMC Whitelist Cancelled",
                                 description="If you would like to start a new application please use !apply",
                                 color=0x00ff00)

# Menus
# Application Menu
class applicationMenu(menus.Menu):
    async def send_initial_message(self, ctx, channel):
        applicant = ctx.author
        return await applicant.send(embed=applicationReady)
    ...
    async def on_thumbs_down(self, payload):
        await self.message.edit(embed=applicationCancelled)
...
async def apply2(ctx):
    print(ctx.author, 'is applying')
    appsMenus = applicationMenu()
    await appsMenus.start(ctx)
...
embedVar = discord.Embed(title="Apply For SaharaMC Whitelist",
                         description="This is the application for SaharaMC whitelist", color=0x00ff00)
...
embedVar.add_field(
    name="What is your favourite part of Minecraft and what do you look forward to doing the most if accepted?",
    value="Favourite part of Minecraft", inline=True)
embedVar.add_field(
    name="Please explain a little bit about yourself. Maybe you have something unique that you would like to share. (3-4 sentences minimum please)",
    value="A little about yourself", inline=True)
embedVar.add_field(
    name="What will you bring to the community? if you have any great builds you can post a link here. (3-4 sentences minimum please)",
    value="What will you bring", inline=True)
embedVar.add_field(name="How did you hear about us? If from a website, please enter the website name.",
                   value="Hear about us", inline=True)
embedVar.add_field(
    name="You do understand SaharaMC is built on trust and that is why we emphasize the importance of the rules. Have you completely read all of the rules? Enter yes or no?",
    value="Acceptance", inline=True)
